...
router.get("/watch-condition/:id", isLoggedIn, checkId, async (req: ConveyorRequest, res) => {
  try {
...
router.post(
  "/watch-condition/:directoryId",
  isLoggedIn,
  checkDirectoryId,
  forceJSON,
  async (req: ConveyorRequest, res) => {
    try {
...
router.patch("/watch-condition/:id", isLoggedIn, checkId, forceJSON, async (req: ConveyorRequest, res) => {
  try {
    const id = Number(req.params.id);
    const condition = await req.db!.getWatchCondition(id);
    if (!condition) {
...
router.delete("/watch-condition/:id", isLoggedIn, checkId, async (req: ConveyorRequest, res) => {
  try {
    const id = Number(req.params.id);
    const condition = await req.db!.getWatchCondition(id);
    if (!condition) {
...
router.get("/watch-condition-preset", isLoggedIn, async (req: ConveyorRequest, res) => {
  try {
...
router.get("/watch-condition-preset/:id", isLoggedIn, checkId, async (req: ConveyorRequest, res) => {
  try {
    const id = Number(req.params.id);
...
router.post("/watch-condition-preset", isLoggedIn, forceJSON, async (req: ConveyorRequest, res) => {
  try {
...
router.patch("/watch-condition-preset/:id", isLoggedIn, checkId, forceJSON, async (req: ConveyorRequest, res) => {
  try {
    const id = Number(req.params.id);
    const preset = await req.db!.getWatchConditionPreset(id);
    if (!preset) {
...
router.delete("/watch-condition-preset/:id", isLoggedIn, checkId, async (req: ConveyorRequest, res) => {
  try {
    const id = Number(req.params.id);
    const preset = await req.db!.getWatchConditionPreset(id);
    if (!preset) {
