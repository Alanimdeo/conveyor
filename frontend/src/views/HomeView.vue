<template>
  ...
  <div class="footer small gray">
    ...
    <span v-else-if="updateAvailable">
      업데이트 가능! 최신 버전:
      <a href="https://github.com/Alanimdeo/conveyor/releases/latest" target="_blank" class="gray">
        {{ latestVersion }}
      </a>
    </span>
    <span v-else>
...
const latestVersion = await fetch("https://api.github.com/repos/Alanimdeo/conveyor/tags")
  .then(async (res) => {
    const tags: string[] = (await res.json()).map((tag: { name: string }) => tag.name);
